...
- (void)dealloc
{
    [tlsSettings release],tlsSettings = nil;
    [super dealloc];
}
...
@implementation GCDAsyncSocket
{
	...
	id delegate;//    此处__weak已删掉 by Gehry
...
- (void)setPortItem:(IMBPortItem *)portItem {
    _portItem = portItem;
}
...
- (void)dealloc
{
    ...
    [readQueue release],readQueue = nil;
    [writeQueue release],writeQueue = nil;
    [preBuffer release], preBuffer = nil;
    if (sslPreBuffer != nil) {
        [sslPreBuffer release];
        sslPreBuffer = nil;
    }
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			GCDAsyncSocket *weakSelf = self;//    此处__weak已删掉 by Gehry
...
- (NSData *)connectedAddress
{
	...
	dispatch_block_t block = ^{
		if (socket4FD != SOCKET_NULL)
		{
			...
			{
				result = [[[NSData alloc] initWithBytes:&sockaddr4 length:sockaddr4len] autorelease];
			}
		}
		...
		if (socket6FD != SOCKET_NULL)
		{
			...
			{
				result = [[[NSData alloc] initWithBytes:&sockaddr6 length:sockaddr6len] autorelease];
			}
		}
	};
