...
- (id)initWithIPod:(IMBiPod*)ipod {
    ...
    if (self) {
        ...
        if (_isServiceBackup) {
            _backupFolderPath = [[iPod.exportSetting backupPath] retain];
        }else {
            _backupFolderPath = [[TempHelper getBackupFolderPath] retain];
        }
        if (![fm fileExistsAtPath:[TempHelper getBackupFolderPath]]) {
            [fm createDirectoryAtPath:[TempHelper getBackupFolderPath] withIntermediateDirectories:YES attributes:nil error:nil];
        }
...
- (void)backUp {
    @autoreleasepool {
        ...
        if ([fm fileExistsAtPath:_deviceBackupPath]) {
            NSString *toPath = [[TempHelper getBackupFolderPath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@-%@", deviceHandle.udid, lbDate]];
            [dateFormatter release];
            dateFormatter = nil;
            [fm moveItemAtPath:_deviceBackupPath toPath:toPath error:nil];
        }
...
- (void)backupNSTask {
    ...
    if ([self checkBackupEncrypt]) {
        ...
        [nc postNotificationName:NOTIFY_BACKUP_ERROR object:CustomLocalizedString(@"backup_id_text_10", nil) userInfo:nil];
        [_logHandle writeErrorLog:CustomLocalizedString(@"backup_id_text_10", nil)];
    } else {
        ...
        NSArray *arguments = [NSArray arrayWithObjects: @"--backup", uuid, nil];
...
- (void)restoreNSTask:(NSString *)sourceUUID restoreType:(NSString *)restoreType{
    ...
    if ([TempHelper stringIsNilOrEmpty:sourceUUID]) {
...
- (void)backupOutData:(NSNotification *)notification {
    //[_logHandle writeErrorLog:@"backupOutData in"];
    ...
    NSString *errStr = nil;
    if (![TempHelper stringIsNilOrEmpty:str] && [str rangeOfString:@"ERROR:"].length > 0) {
    ...
    if (![TempHelper stringIsNilOrEmpty:errStr] && [str rangeOfString:@"Request name missing from message:"].length > 0) {
        ...
        if (![TempHelper stringIsNilOrEmpty:errorCode]) {
            NSDictionary *userDic = [NSDictionary dictionaryWithObjectsAndKeys:
                                     [TempHelper stringIsNilOrEmpty:errorReason] ? @"": errorReason, NOTIFY_ERROR_REASON,
            ...
            if (![TempHelper stringIsNilOrEmpty:errorReason]) {
    ...
    } else if (![TempHelper stringIsNilOrEmpty:errStr] && [errStr rangeOfString:@"MBErrorDomain"].length > 0) {
        errorReason = errStr;
        [_logHandle writeErrorLog:[NSString stringWithFormat:@"MBErrorDomain errorReason:%@", errorReason]];
    } else if ([TempHelper stringIsNilOrEmpty:errStr]) {
        ...
        @try {
            ...
            if ([str rangeOfString:@"<split>"].length > 0) {
                ...
                for (NSString *strItem in progressStrArray) {
                    if ([TempHelper stringIsNilOrEmpty:strItem]) {
                        continue;
                    }
...
- (BOOL) checkExistSnapshot {
    ...
    NSString *backupFolder = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Application Support/MobileSync/Backup"];
    NSString *backupFilePath = [[backupFolder stringByAppendingPathComponent:[iPod.deviceHandle udid]] stringByAppendingPathComponent:@"Snapshot"];
    return [[NSFileManager defaultManager] fileExistsAtPath:backupFilePath];
}
...
- (void)backupFinished:(NSNotification *)notification {
    ...
    if (backupFinished) {
        ...
        NSString *infoLocalPath = [[[self getBackupFolderPath] stringByAppendingPathComponent:[deviceHandle udid]] stringByAppendingPathComponent:@"Info.plist"];
...
- (void)restoreOutData:(NSNotification *)notification {
    ...
    if (![TempHelper stringIsNilOrEmpty:errStr] && [errStr rangeOfString:@"Request name missing from message:"].length > 0) {
        NSString *tmpStr = [errStr substringFromIndex:([errStr rangeOfString:@"Request name missing from message:"].location + @"Request name missing from message:".length)];
        NSDictionary *dic = [self dictionaryWithString:tmpStr];
        NSString *errorCode = nil;
        if (dic != nil && dic.allKeys.count > 0) {
...
- (NSString *)stringWithDictionary:(NSDictionary *)dict {
    NSData *dicData = [NSPropertyListSerialization dataWithPropertyList:dict
...
- (void)writePlistFile:(NSString*)filePath {
    ...
    // 获取设备的基本的属性值
    NSString *buildVersion = [deviceHandle deviceValueForKey:@"BuildVersion"];
    if (buildVersion != nil && ![buildVersion isEqualToString:@""] ) {
        [infoPlist setValue:buildVersion forKey:@"Build Version"];
    }
    NSString *deviceName = [deviceHandle deviceName];
    ...
    /*NSString *udid = [deviceHandle deviceValueForKey:@"UniqueDeviceID"];
     NSLog(@"udid is %@", udid);*/
    NSString *ICCID = [deviceHandle deviceValueForKey:@"IntegratedCircuitCardIdentity"];
    if (ICCID != nil && ![ICCID isEqualToString:@""] ) {
        [infoPlist setValue:ICCID forKey:@"ICCID"];
    }
    NSString *IMEI = [deviceHandle deviceValueForKey:@"InternationalMobileEquipmentIdentity"];
    ...
    NSString *phoneNumber = [deviceHandle deviceValueForKey:@"PhoneNumber"];
    if (phoneNumber != nil && ![phoneNumber isEqualToString:@""] ) {
        [infoPlist setValue:phoneNumber forKey:@"Phone Number"];
    }
    NSString *phonerescueVersion = [[IMBSoftWareInfo singleton] version];
    [infoPlist setValue:phonerescueVersion forKey:@"AnyTrans Version"];
    NSString *productType = [deviceHandle deviceValueForKey:@"ProductType"];
    if (productType != nil && ![productType isEqualToString:@""] ) {
        [infoPlist setValue:productType forKey:@"Product Type"];
    }
    NSString *productVersion = [deviceHandle deviceValueForKey:@"ProductVersion"];
    if (productVersion != nil && ![productVersion isEqualToString:@""] ) {
        [infoPlist setValue:productVersion forKey:@"Product Version"];
    }
    NSString *serialNumber = [deviceHandle deviceValueForKey:@"SerialNumber"];
    if (serialNumber != nil && ![serialNumber isEqualToString:@""] ) {
        [infoPlist setValue:serialNumber forKey:@"Serial Number"];
    }
    NSString *targetIdentifier = [deviceHandle deviceValueForKey:@"UniqueDeviceID"];
