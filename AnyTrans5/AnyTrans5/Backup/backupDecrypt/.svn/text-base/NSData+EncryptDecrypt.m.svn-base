...
+ (NSMutableData *)Rfc2898DeriveBytes256:(NSString *)passcode withSalt:(NSData *)salt withIterations:(int)iterations {
    ...
    NSMutableData *rfc1898Data = [[NSMutableData alloc] initWithLength:(kCCKeySizeAES256 + kCCBlockSizeAES128)];
    [IMBRfc2898DeriveBytes deriveBytes256:rfc1898Data fromPassword:passcode withSalt:salt withIterations:iterations];
    if (rfc1898Data != nil && rfc1898Data.length >= 32) {
        memcpy(key, rfc1898Data.bytes, 32);
    } else {
        return nil;
