//
//  IMBFastDriverTreeViewController.m
//  AnyTrans
//
...
@interface IMBFastDriverTreeViewController ()

@end

@implementation IMBFastDriverTreeViewController
...
-(void)setAllselectState:(CheckStateEnum)sender{
    ...
    for (int i=0;i<[_dataSourceArray count]; i++) {
        SimpleNode *node = [_dataSourceArray objectAtIndex:i];
        if (!node.isCoping) {
...
- (void)doublClick:(id)sender
{
    ...
    if (row <[_dataSourceArray count]) {
        SimpleNode *node = [_dataSourceArray objectAtIndex:row];
        if (node.container) {
    ...
    if ([_delegate respondsToSelector:@selector(treeDoubleClick:)]) {
        [_delegate treeDoubleClick:row];
    }
}
...
- (void)reload:(id)sender
{
    ...
    for (int i=0; i<[self.dataSourceArray count];i++) {
        SimpleNode *node = [self.dataSourceArray objectAtIndex:i];
        if (node.isCoping) {
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if ([@"Size" isEqualToString:tableColumn.identifier]) {
        if (node.container) {
            ...
            return [StringHelper getFileSizeString:node.itemSize reserved:2];
        }
    }
...
- (void)tableViewSelectionDidChange:(NSNotification *)aNotification {
    ...
    for (int i=0; i<[displayArray count]; i++) {
        SimpleNode *node = [displayArray objectAtIndex:i];
    ...
    if ([set count] > 0) {
        SimpleNode *node = [displayArray objectAtIndex:[set lastIndex]];
    ...
    if ([_delegate respondsToSelector:@selector(setSelectWord: withSelectCount:)]) {
        [_delegate setSelectWord:[_dataSourceArray count] withSelectCount:[set count]];
    }
}
...
- (void)tableView:(NSTableView *)tableView row:(NSInteger)index {
    ...
    SimpleNode *node = [displayArray objectAtIndex:index];
    if (node.isCoping) {
        return;
    }
    ...
    NSMutableIndexSet *set = [NSMutableIndexSet indexSet];
    for (int i=0;i<[_dataSourceArray count]; i++) {
        SimpleNode *item= [_dataSourceArray objectAtIndex:i];
