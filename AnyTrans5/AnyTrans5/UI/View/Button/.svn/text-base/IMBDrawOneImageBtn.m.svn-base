...
//    [clipPath fill];
    ...
    if (_longTimeDown) {
     ...
     if (_isDFUGuide) {
        ...
        NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[StringHelper getColorFromString:CustomColor(@"text_numberColor", nil)],NSForegroundColorAttributeName ,paragraphStyle,NSParagraphStyleAttributeName,[NSFont fontWithName:@"Helvetica Neue Light" size:28],NSFontAttributeName, nil];
        ...
        NSMutableParagraphStyle *paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
        [paragraphStyle setAlignment:NSCenterTextAlignment];
        NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[StringHelper getColorFromString:CustomColor(@"tableView_select_FocusColor", nil)],NSForegroundColorAttributeName ,paragraphStyle,NSParagraphStyleAttributeName,[NSFont fontWithName:@"Helvetica Neue Light" size:28],NSFontAttributeName, nil];
        NSSize textSize = [as.string sizeWithAttributes:attributes];
        NSRect f;
        f = NSMakeRect((dirtyRect.size.width - textSize.width)/2, (dirtyRect.size.height - textSize.height)/2-1, textSize.width, textSize.height);
        [as.string drawInRect:f withAttributes:attributes];
