...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    if (_isselected) {
        ...
        [[StringHelper getColorFromString:CustomColor(@"tableView_select_FocusColor", nil)] setFill];
        [backgroundpath fill];
     }else
     {
         NSBezierPath *backgroundpath = [NSBezierPath bezierPathWithRoundedRect:rect xRadius:8 yRadius:8];
         if (string != nil) {
             [attiStr addAttributes:[self attributed:[StringHelper getColorFromString:CustomColor(@"text_normalColor", nil)]]  range:NSMakeRange(0, string.length)];
        }
