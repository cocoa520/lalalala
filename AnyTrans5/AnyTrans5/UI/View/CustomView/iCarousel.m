...
- (void)setUp
{
    ...
    _scrollEnabled = YES;//用于设置是否可以用鼠标拖动，视图变化
    _bounces = YES;
...
- (NSInteger)indexOfItemViewOrSubview:(UIView *)view
{
    ...
    {
        return [self indexOfItemViewOrSubview:(UIView *)view.superview];
    }
    return index;
}
...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeLinear:
        {
            ...
            {
                static CGFloat max_sacle = 1.0f;//放大倍数
                
                static CGFloat normal_scale = 0.8f;//一般大小
                
                if (offset <= 1 && offset >= -1) {
                    
                    float tempScale = offset < 0 ? 1+offset : 1-offset;
                    
                    float slope = (max_sacle - normal_scale) / 1;
                    
                    CGFloat scale = normal_scale + slope * tempScale;
                    
                    transform = CATransform3DScale(transform, scale, scale, 1);
                    
                }else{
                    transform = CATransform3DScale(transform, normal_scale, normal_scale,1);
                }
...
//    {
        [self scrollToItemAtIndex:index animated:(animated && _numberOfPlaceholders)];
//    }
}
