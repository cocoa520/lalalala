...
- (ECPoint*)detach {
    return [[[SecT409R1Point alloc] initWithCurve:nil withX:[self affineXCoord] withY:[self affineYCoord]] autorelease];
}
    ...
    @autoreleasepool {
        ...
        if ([B isZero]) {
            ...
            if ([X2 isZero]) {
                ...
                if ([X3 isZero]) {
                    retVal = [[[SecT409R1Point alloc] initWithCurve:curve withX:X3 withY:[[curve b] sqrt] withCompression:[self isCompressed]] autorelease];
                    goto jumpTo;
                }
            ...
            retVal = [[[SecT409R1Point alloc] initWithCurve:curve withX:X3 withY:L3 withZS:zs withCompression:self.isCompressed] autorelease];
#if !__has_feature(objc_arc)
...
- (ECPoint*)twice {
    ...
    @autoreleasepool {
        ...
        if ([T isZero]) {
            retVal = [[SecT409R1Point alloc] initWithCurve:curve withX:T withY:[[curve b] sqrt] withCompression:[self isCompressed]];
        } else {
            ...
            NSMutableArray *zs = [[NSMutableArray alloc] initWithObjects:Z3, nil];
            retVal = [[SecT409R1Point alloc] initWithCurve:curve withX:X3 withY:L3 withZS:zs withCompression:self.isCompressed];
#if !__has_feature(objc_arc)
            if (zs != nil) [zs release]; zs = nil;
#endif
        }
    ...
    @autoreleasepool {
        ...
        if ([X2 isZero] || ![Z2 isOne]) {
            ...
            if ([B isZero]) {
                ...
                if ([A isZero]) {
                    retVal = [[[SecT409R1Point alloc] initWithCurve:curve withX:A withY:[[curve b] sqrt] withCompression:[self isCompressed]] autorelease];
                } else {
...
- (ECPoint*)negate {
    ...
    @autoreleasepool {
        ...
        retVal = [[SecT409R1Point alloc] initWithCurve:[self curve] withX:X withY:[L add:Z] withZS:zs withCompression:[self isCompressed]];
#if !__has_feature(objc_arc)
        if (zs != nil) [zs release]; zs = nil;
#endif
    }
