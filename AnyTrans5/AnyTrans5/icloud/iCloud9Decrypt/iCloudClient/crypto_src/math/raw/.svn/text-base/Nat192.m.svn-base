...
+ (NSMutableArray*)create {
    return [[NSMutableArray alloc] initWithSize:6];
}

// return == uint64_t[3]
+ (NSMutableArray*)create64 {
    return [[NSMutableArray alloc] initWithSize:3];
}

// return == uint[12]
+ (NSMutableArray*)createExt {
    return [[NSMutableArray alloc] initWithSize:12];
}
...
+ (BOOL)diff:(NSMutableArray*)x withXoff:(int)xOff withY:(NSMutableArray*)y withYoff:(int)yOff withZ:(NSMutableArray*)z withZoff:(int)zOff {
    BOOL pos = [Nat192 gte:x withXoff:xOff withY:y withYoff:yOff];
    if (pos) {
        [Nat192 sub:x withXoff:xOff withY:y withYoff:yOff withZ:z withZoff:zOff];
    } else {
        [Nat192 sub:y withXoff:yOff withY:x withYoff:xOff withZ:z withZoff:zOff];
    }
    return pos;
}
...
+ (uint)mulAddTo:(NSMutableArray*)x withXoff:(int)xOff withY:(NSMutableArray*)y withYoff:(int)yOff withZZ:(NSMutableArray*)zz withZZoff:(int)zzOff {
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            zz[zzOff + 5] = @((uint)c);
            c >>= 32;
            c += zc + [zz[zzOff + 6] unsignedIntValue];
            zz[zzOff + 6] = @((uint)c);
            zc = c >> 32;
            ++zzOff;
        }
...
+ (uint)mulWordAddExt:(uint)x withYY:(NSMutableArray*)yy withYYoff:(int)yyOff withZZ:(NSMutableArray*)zz withZZoff:(int)zzOff {
    ...
    @autoreleasepool {
        ...
        zz[zzOff + 3] = @((uint)c);
        c >>= 32;
        c += xVal * [yy[yyOff + 4] unsignedIntValue] + [zz[zzOff + 4] unsignedIntValue];
        zz[zzOff + 4] = @((uint)c);
        c >>= 32;
        c += xVal * [yy[yyOff + 5] unsignedIntValue] + [zz[zzOff + 5] unsignedIntValue];
        zz[zzOff + 5] = @((uint)c);
        c >>= 32;
    }
    return (uint)c;
...
+ (uint)mul33DWordAdd:(uint)x withY:(uint64_t)y withZ:(NSMutableArray*)z withZoff:(int)zOff {
    ...
    @autoreleasepool {
        ...
        z[zOff + 3] = @((uint)c);
        c >>= 32;
        retVal = (c == 0 ? 0 : [Nat incAt:6 withZ:z withZoff:zOff withZpos:4]);
    }
    return retVal;
}
...
+ (uint)mul33WordAdd:(uint)x withY:(uint)y withZ:(NSMutableArray*)z withZoff:(int)zOff {
    ...
    @autoreleasepool {
        ...
        z[zOff + 2] = @((uint)c);
        c >>= 32;
        retVal = (c == 0 ? 0 : [Nat incAt:6 withZ:z withZoff:zOff withZpos:3]);
    }
    return retVal;
}
...
+ (void)square:(NSMutableArray*)x withZZ:(NSMutableArray*)zz {
    @autoreleasepool {
        ...
        zz[10] = @((w << 1) | c);
        c = w >> 31;
        w = [zz[11] unsignedIntValue] + (uint)(zz_10 >> 32);
        zz[11] = @((w << 1) | c);
    }
}
...
+ (void)square:(NSMutableArray*)x withXoff:(int)xOff withZZ:(NSMutableArray*)zz withZZoff:(int)zzOff {
    @autoreleasepool {
        ...
        zz[zzOff + 10] = @((w << 1) | c);
        c = w >> 31;
        w = [zz[zzOff + 11] unsignedIntValue] + (uint)(zz_10 >> 32);
        zz[zzOff + 11] = @((w << 1) | c);
    }
}
...
+ (BigInteger*)toBigInteger:(NSMutableArray*)x {
    ...
    @autoreleasepool {
        NSMutableData *bs = [[NSMutableData alloc] initWithSize:24];
        for (int i = 0; i < 6; ++i) {
            uint x_i = [x[i] unsignedIntValue];
            if (x_i != 0) {
                [Pack UInt32_To_BE:x_i withBs:bs withOff:((5 - i) << 2)];
            }
        }
...
+ (BigInteger*)toBigInteger64:(NSMutableArray*)x {
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (x_i != 0L) {
                [Pack UInt64_To_BE:x_i withBs:bs withOff:((2 - i) << 3)];
            }
        }
        retVal = [[BigInteger alloc] initWithSign:1 withBytes:bs];
#if !__has_feature(objc_arc)
