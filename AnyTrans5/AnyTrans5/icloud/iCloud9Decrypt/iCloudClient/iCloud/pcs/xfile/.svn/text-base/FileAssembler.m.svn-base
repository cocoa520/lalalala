...
- (BOOL)assemble:(NSString*)path withAsset:(AssetEx*)asset withChunks:(NSMutableArray*)chunks withCancel:(BOOL*)cancel {
    ...
    if (u) {
        retVal = [self decrypt:path withInfo:info withChunks:chunks withEncryptionKey:u withSignature:[asset getFileChecksum] withCompression:[asset getContentCompressionMethod] withCancel:cancel];
    } else {
        retVal = [self write:path withInfo:info withChunks:chunks withKeyCipher:nil withSignature:[asset getFileChecksum] withCompression:[asset getContentCompressionMethod] withCancel:cancel];
    }
    return retVal;
}
 ...
 *
 *  @param compression     多了该参数
 *
 */
- (BOOL)decrypt:(NSString*)path withInfo:(NSString*)info withChunks:(NSMutableArray*)chunks withEncryptionKey:(NSMutableData*)encryptionKey withSignature:(NSMutableData*)signature withCompression:(NSNumber *)compression withCancel:(BOOL*)cancel {
    if (*cancel) {
        return NO;
    }
    BOOL retVal = NO;
    ...
    if (xfk) {
        retVal = [self write:path withInfo:info withChunks:chunks withKeyCipher:xfk withSignature:signature withCompression:compression withCancel:cancel];
    } else {
        retVal = NO;
    }
    return retVal;
 ...
 *
 */
- (BOOL)write:(NSString*)path withInfo:(NSString*)info withChunks:(NSMutableArray*)chunks withKeyCipher:(XFileKey*)keyCipher withSignature:(NSMutableData*)signature withCompression:(NSNumber *)compression withCancel:(BOOL*)cancel {
    if (*cancel) {
        return NO;
    }
    Stream *decompress = nil;
    @try {
        NSFileManager *fm = [NSFileManager defaultManager];
        ...
        if (ret) {
            ...
            status = [FileStreamWriter copy:inputStream withOutStream:outputStream withKeyCipher:keyCipher withSignature:signature withDecompress:decompress withCancel:cancel];
            [outputStream closeFile];
            if (compression) {
                if ([compression intValue] == 2) {
                    if ([FileAssembler LZFSE]) {
                        BOOL success = [self decodeInput:path withOutputPath:path];
                        if (success) {
                            decompress = nil;
                        }
                    }else {
                        NSLog(@"-- write() - no  decompressor: %@ -> %d", info, [compression intValue]);
                        decompress = nil;
                    }
                }else {
                    NSLog(@"-- write() - unsupported compression: %@ -> %d", info, [compression intValue]);
                    decompress = nil;
                }
            }else {
                decompress = nil;
 ...
 *  Patched in iOS 11 Support
 */
- (BOOL)decodeInput:(NSString *)inputPath withOutputPath:(NSString *)outputPath {
    NSMutableArray *params = [NSMutableArray arrayWithObjects:@"-decode", @"-i", [NSString stringWithFormat:@"%@", inputPath], @"-o", [NSString stringWithFormat:@"%@", outputPath], nil];
    return [self runLZFSE:params];
}

- (BOOL)runLZFSE:(NSArray*)params {
    NSString* ffmpegPath = [[NSBundle mainBundle] pathForResource:@"lzfse" ofType:@""];
