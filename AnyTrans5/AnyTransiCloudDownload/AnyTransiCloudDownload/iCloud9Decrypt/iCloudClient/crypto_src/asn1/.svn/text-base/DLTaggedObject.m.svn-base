...
- (BOOL)isConstructed {
    if (!self.empty) {
        ...
        ASN1Primitive *localASN1Primitive = [[self.obj toASN1Primitive] toDLObject];
...
- (int)encodedLength {
    if (!self.empty) {
        int i = [[[self.obj toASN1Primitive] toDLObject] encodedLength];
        if (self.explicit) {
...
- (void)encode:(ASN1OutputStream *)paramASN1OutputStream {
    if (!self.empty) {
        ...
        [paramASN1OutputStream writeEncoded:160 paramInt2:self.tagNO paramArrayOfByte:[DLTaggedObject ZERO_BYTES]];
    }
}
