...
@implementation TBSCertificateStructure
@synthesize seq = _seq;
...
+ (TBSCertificateStructure *)getInstance:(id)paramObject {
    if ([paramObject isKindOfClass:[TBSCertificateStructure class]]) {
        return (TBSCertificateStructure *)paramObject;
    }
    if (paramObject) {
        return [[[TBSCertificateStructure alloc] initParamASN1Sequence:[ASN1Sequence getInstance:paramObject]] autorelease];
    }
    return nil;
}

+ (TBSCertificateStructure *)getInstance:(ASN1TaggedObject *)paramASN1TaggedObject paramBoolean:(BOOL)paramBoolean {
    return [TBSCertificateStructure getInstance:[ASN1Sequence getInstance:paramASN1TaggedObject paramBoolean:paramBoolean]];
}
        ...
        for (int j = (int)[paramASN1Sequence size] - (i + 6) - 1; j > 0; j--) {
            ...
            switch ([localDERTaggedObject getTagNo]) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    self.extensions = [X509Extensions getInstance:localDERTaggedObject];
                default:
                    break;
            }
        }
