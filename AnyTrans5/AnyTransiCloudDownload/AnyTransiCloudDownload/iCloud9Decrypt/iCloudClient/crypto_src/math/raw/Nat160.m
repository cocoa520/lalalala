...
+ (BOOL)diff:(NSMutableArray*)x withXoff:(int)xOff withY:(NSMutableArray*)y withYoff:(int)yOff withZ:(NSMutableArray*)z withZoff:(int)zOff {
    BOOL pos = [Nat160 gte:x withXoff:xOff withY:y withYoff:yOff];
    if (pos) {
        [Nat160 sub:x withXoff:xOff withY:y withYoff:yOff withZ:z withZoff:zOff];
    } else {
        [Nat160 sub:y withXoff:yOff withY:x withYoff:xOff withZ:z withZoff:zOff];
    }
    return pos;
}
...
+ (uint)mulAddTo:(NSMutableArray*)x withXoff:(int)xOff withY:(NSMutableArray*)y withYoff:(int)yOff withZZ:(NSMutableArray*)zz withZZoff:(int)zzOff {
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            zz[zzOff + 4] = @((uint)c);
            c >>= 32;
            c += zc + [zz[zzOff + 5] unsignedIntValue];
            zz[zzOff + 5] = @((uint)c);
            zc = c >> 32;
            ++zzOff;
        }
...
+ (uint)mul33DWordAdd:(uint)x withY:(uint64_t)y withZ:(NSMutableArray*)z withZoff:(int)zOff {
    ...
    @autoreleasepool {
        ...
        z[zOff + 3] = @((uint)c);
        c >>= 32;
        retVal = (c == 0 ? 0 : [Nat incAt:5 withZ:z withZoff:zOff withZpos:4]);
    }
    return retVal;
}
...
+ (uint)mul33WordAdd:(uint)x withY:(uint)y withZ:(NSMutableArray*)z withZoff:(int)zOff {
    ...
    @autoreleasepool {
        ...
        z[zOff + 2] = @((uint)c);
        c >>= 32;
        retVal = (c == 0 ? 0 : [Nat incAt:5 withZ:z withZoff:zOff withZpos:3]);
    }
    return retVal;
}
...
+ (uint)mulWordsAdd:(uint)x withY:(uint)y withZ:(NSMutableArray*)z withZoff:(int)zOff {
    ...
    @autoreleasepool {
        ...
        z[zOff + 1] = @((uint)c);
        c >>= 32;
        retVal = (c == 0 ? 0 : [Nat incAt:5 withZ:z withZoff:zOff withZpos:2]);
    }
    return retVal;
}
...
+ (void)square:(NSMutableArray*)x withZZ:(NSMutableArray*)zz {
    @autoreleasepool {
        ...
        zz[8] = @((w << 1) | c);
        c = w >> 31;
        w = [zz[9] unsignedIntValue] + (uint)(zz_8 >> 32);
        zz[9] = @((w << 1) | c);
    }
}
...
+ (void)square:(NSMutableArray*)x withXoff:(int)xOff withZZ:(NSMutableArray*)zz withZZoff:(int)zzOff {
    @autoreleasepool {
        ...
        zz[zzOff + 8] = @((w << 1) | c);
        c = w >> 31;
        w = [zz[zzOff + 9] unsignedIntValue] + (uint)(zz_8 >> 32);
        zz[zzOff + 9] = @((w << 1) | c);
    }
}
...
+ (BigInteger*)toBigInteger:(NSMutableArray*)x {
    ...
    @autoreleasepool {
        NSMutableData *bs = [[NSMutableData alloc] initWithSize:20];
        for (int i = 0; i < 5; ++i) {
            uint x_i = [x[i] unsignedIntValue];
            if (x_i != 0) {
                [Pack UInt32_To_BE:x_i withBs:bs withOff:((4 - i) << 2)];
            }
        }
