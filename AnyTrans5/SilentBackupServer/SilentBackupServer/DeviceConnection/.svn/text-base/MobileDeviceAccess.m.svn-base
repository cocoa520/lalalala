...
mach_error_t AMDeviceSecureStartService(am_device device, CFStringRef service_name, CFDictionaryRef unknown, am_service *handle);//++++++++++
...
uint32_t AFCConnectionSetSecureContext(afc_connection conn, uint32_t wifi);
uint32_t AMDServiceConnectionGetSecureIOContext(afc_long socketid);
...
- (id)initWithAMDevice:(AMDevice*)device
{
	...
	if (self = [super initWithName:@"com.apple.afc" onDevice:device isSecure:isSecure]) {
		int ret = AFCConnectionOpen(socket_fd, 0/*timeout*/, &_afc);
		if (ret != 0) {
        ...
        if (isSecure) {
            uint32_t secureIOContext = AMDServiceConnectionGetSecureIOContext(_service);
            if (secureIOContext == 0) {
                [self release];
                self = nil;
            }
            ret = AFCConnectionSetSecureContext(_afc, secureIOContext);
            if (ret != 0) {
                [self release];
                self = nil;
            }
...
- (int)checkDeviceConnectStatus {
    ...
    am_service result;
//	uint32_t dummy = 0;
    ret = AMDeviceSecureStartService(_device, (CFStringRef)@"com.apple.afc", 0, &result);//AMDeviceStartService(_device,(CFStringRef)@"com.apple.afc", &result, &dummy);--有线连接
    if (ret != 0)
    {
                ...
                if ([_listener canSupportWifi]) {
                    ...
                    if (d != nil) {
                        ...
                        if (_listener && [_listener respondsToSelector:@selector(deviceNeedPassword:)]) {
                            [_listener deviceNeedPassword:info->dev];
                        }
                    }
                }
            } else {
