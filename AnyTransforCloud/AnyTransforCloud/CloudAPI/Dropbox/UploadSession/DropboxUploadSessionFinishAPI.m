...
- (NSDictionary<NSString *,NSString *> *)requestHeaderFieldValueDictionary
{
    ...
    if ([_parent rangeOfString:@"0"].location != NSNotFound) {
        NSString *commit = [NSString stringWithFormat:@"{\"path\":\"/%@\", \"autorename\": true}", [BaseDriveAPI zhAndJpToUnicode:_fileName]];
        uploadItem = [NSString stringWithFormat:@"{\"cursor\": %@, \"commit\" : %@}", cursor, commit];
    }else {
        NSString *commit = [NSString stringWithFormat:@"{\"path\":\"/%@/%@\", \"autorename\": true}", _parent, _fileName];
        commit = [BaseDriveAPI zhAndJpToUnicode:commit];
        uploadItem = [NSString stringWithFormat:@"{\"cursor\": %@, \"commit\" : %@}", cursor, commit];
    }
    return @{
