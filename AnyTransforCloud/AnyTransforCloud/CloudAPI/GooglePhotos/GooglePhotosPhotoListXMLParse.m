//
//  GooglePhotoPhotoListXMLParse.m
//  DriveSync
//
...
@implementation GooglePhotosPhotoListXMLParse
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(nullable NSString *)namespaceURI qualifiedName:(nullable NSString *)qName attributes:(NSDictionary<NSString *, NSString *> *)attributeDict
{
    if ([elementName isEqualToString:@"entry"])
    {
        ...
        [self.currentDic addEntriesFromDictionary:@{@"type":[attributeDict objectForKey:@"type"]}];
    }else if ([elementName isEqualToString:@"media:content"]){
        [self.currentDic addEntriesFromDictionary:@{@"url":[attributeDict objectForKey:@"url"]}];
    }else if ([elementName isEqualToString:@"media:thumbnail"]){
        NSMutableArray *thumbnailurlArray = [self.currentDic objectForKey:@"thumbnailurl"];
        if (thumbnailurlArray == nil) {
            thumbnailurlArray = [NSMutableArray array];
        }
        [thumbnailurlArray addObject:[attributeDict objectForKey:@"url"]];
        
        [self.currentDic addEntriesFromDictionary:@{@"thumbnailurl":thumbnailurlArray}];
