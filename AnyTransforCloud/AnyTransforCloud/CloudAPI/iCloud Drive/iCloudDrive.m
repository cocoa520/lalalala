...
{
    YTKRequest *requestAPI = [[iCloudDriveDeleteItemsAPI alloc] initWithDeleteItems:idOrPathArray dsid:_dsid cookie:_cookie iCloudDriveURL:_iCloudDriveUrl];
    __block YTKRequest *weakRequestAPI = requestAPI;
    [requestAPI startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ResponseCode code = [self checkResponseTypeWithSuccess:request];
        if (code == ResponseSuccess) {
...
- (void)copyToNewParentIDOrPath:(NSString *)newParentIdOrPath idOrPathArray:(NSArray *)idOrPathArray success:(Callback)success fail:(Callback)fail {
    if ([idOrPathArray count] > 0) {
        YTKRequest *requestAPI = [[iCloudDriveCopyItemAPI alloc] initWithCopyItemDic:idOrPathArray newParentIDOrPathdsid:newParentIdOrPath dsid:_dsid cookie:_cookie iCloudDriveURL:_iCloudDriveUrl];
        __block YTKRequest *weakRequestAPI = requestAPI;
        [requestAPI startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
            ResponseCode code = [self checkResponseTypeWithSuccess:request];
            if (code == ResponseSuccess) {
...
- (void)reName:(NSString *)newName idOrPathArray:(NSArray *)idOrPathArray success:(Callback)success fail:(Callback)fail {
    if ([idOrPathArray count] > 0) {
        NSDictionary *itemDict = [idOrPathArray objectAtIndex:0];
        YTKRequest *requestAPI = [[iCloudDriveRenameAPI alloc] initWithRenameItems:itemDict dsid:_dsid cookie:_cookie iCloudDriveURL:_iCloudDriveUrl];
        __block YTKRequest *weakRequestAPI = requestAPI;
        [requestAPI startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
            ResponseCode code = [self checkResponseTypeWithSuccess:request];
            if (code == ResponseSuccess) {
...
- (void)moveToNewParentIDOrPath:(NSString *)newParentIdOrPath idOrPathArray:(NSArray *)idOrPathArray success:(Callback)success fail:(Callback)fail {
    if ([idOrPathArray count] > 0) {
        YTKRequest *requestAPI = [[iCloudDriveMoveToNewParentAPI alloc] initWithMoveItemDic:idOrPathArray newParentIDOrPathdsid:newParentIdOrPath dsid:_dsid cookie:_cookie iCloudDriveURL:_iCloudDriveUrl];
        __block YTKRequest *weakRequestAPI = requestAPI;
        [requestAPI startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
            ResponseCode code = [self checkResponseTypeWithSuccess:request];
            if (code == ResponseSuccess) {
            ...
            if ([resultArray count]>0) {
                ...
                [_upLoader uploadmutilPartItem:item success:^(__kindof YTKBaseRequest * _Nonnull request) {
                    ...
                    if (request.responseStatusCode == 200) {
                        ...
                        YTKRequest *requestThreeAPI = [[iCloudDriveUploadThreeAPI alloc] initWithiCloudDriveDocwsUrl:_iCloudDriveDocwsUrl dataDic:(NSMutableDictionary *)dataDic fileName:[item fileName] parent:[item uploadDocwsID] documentID:documentID cookie:_cookie];
