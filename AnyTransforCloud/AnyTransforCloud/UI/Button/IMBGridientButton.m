...
@synthesize isRightAlignment = _isRightAlignment;
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    if (titleRect.size.width<dirtyRect.size.width-4) {
        ...
        if (_hasLeftImage) {
            ...
            if (_isRightAlignment) {
                 drawRect.origin.x = ceil((dirtyRect.size.width - titleRect.size.width));
            }else {
    ...
    if (color) {
        ...
        if (_hasRightImage) {
            ...
            if (_isAddButton) {
                NSRect pathRect2 = NSMakeRect((dirtyRect.size.width - 12 - 6 - titleRect.size.width)/2, (dirtyRect.size.height - 2)/2, 12, 2);
                NSRect pathRect1 = NSMakeRect(pathRect2.origin.x + 5, (dirtyRect.size.height - 12)/2, 2, 12);
                
                NSRect textRect = NSMakeRect(pathRect2.origin.x + 12 + 6, (dirtyRect.size.height - titleRect.size.height)/2 - 2, titleRect.size.width, titleRect.size.height);
                
                if (_isOpenMenu) {
                    
                    NSBezierPath *path = [NSBezierPath bezierPath];
                    [path moveToPoint:NSMakePoint((dirtyRect.size.width - 12 - 6 - titleRect.size.width)/2, (dirtyRect.size.height - 2)/2 - 3)];
                    [path lineToPoint:NSMakePoint((dirtyRect.size.width - 12 - 6 - titleRect.size.width)/2 + 6, (dirtyRect.size.height - 2)/2 + 5)];
                    [path lineToPoint:NSMakePoint((dirtyRect.size.width - 12 - 6 - titleRect.size.width)/2 + 12, (dirtyRect.size.height - 2)/2 - 3)];
                    ...
                    NSBezierPath *path1 = [NSBezierPath bezierPathWithRoundedRect:pathRect1 xRadius:1 yRadius:1];
                    [[StringHelper getColorFromString:CustomColor(@"mainView_bgColor", nil)] set];
                    [path1 fill];
                    [path1 closePath];
                    
                    NSBezierPath *path2 = [NSBezierPath bezierPathWithRoundedRect:pathRect2 xRadius:1 yRadius:1];
                    [[StringHelper getColorFromString:CustomColor(@"mainView_bgColor", nil)] set];
                    [path2 fill];
                    [path2 closePath];
                }
                ...
                [_buttonTitle drawInRect:textRect withAttributes:dic];
            } else {
