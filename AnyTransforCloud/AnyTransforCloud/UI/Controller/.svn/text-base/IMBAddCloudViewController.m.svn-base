...
@implementation IMBAddCloudViewController
@synthesize dataSourceAryM = _dataSourceAryM;

- (id)initWithDelegate:(id)delegate {
    self = [super initWithNibName:@"IMBAddCloudViewController" bundle:nil];
    if (self) {
...
//    [_dataSourceAryM addObjectsFromArray:array];
    
    NSCollectionViewFlowLayout* flowLayout = [[NSCollectionViewFlowLayout alloc] init];
    flowLayout.itemSize = NSMakeSize(138, 144);
...
- (NSInteger)collectionView:(NSCollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _dataSourceAryM.count;
}

- (NSCollectionViewItem *)collectionView:(NSCollectionView *)collectionView itemForRepresentedObjectAtIndexPath:(NSIndexPath *)indexPath {
    if (_dataSourceAryM.count > 0) {
        IMBCloudCollectionViewItem* item = [_collectionView makeItemWithIdentifier:@"IMBCloudCollectionViewItem" forIndexPath:indexPath];
        IMBCloudEntity *cloud = [_dataSourceAryM objectAtIndex:indexPath.item];
        [item setCloud:cloud];
        item.delegate = self;
        return item;
    }else {
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    if (_personCloudAryM > 0) {
        ...
        if (![StringHelper stringIsNilOrEmpty:drive.driveEmail]) {
            [cellView.cloudPerson setStringValue:drive.driveEmail];
        }else {
            [cellView.cloudPerson setStringValue:@"--"];
        }
        if (![StringHelper stringIsNilOrEmpty:drive.driveName]) {
            [cellView.cloudName setStringValue:drive.displayName];
        }else {
            [cellView.cloudName setStringValue:@"--"];
        }
