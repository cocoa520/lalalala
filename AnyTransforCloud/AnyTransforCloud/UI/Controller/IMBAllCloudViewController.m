...
@implementation IMBAllCloudViewController

- (id)initWithDelegate:(id)delegate withDriveID:(NSString *)driveID {
    if (self = [super initWithDelegate:delegate]) {
        IMBCloudManager *cloudManager = [IMBCloudManager singleton];
        BaseDrive *drive = [[IMBCloudManager singleton] getBindDrive:driveID];
        _baseManager = [[cloudManager getCloudManager:drive] retain];
        _baseManager.delegate = self;
        [_baseManager loadData];
    }
    return self;
}

- (id)initWithDelegate:(id)delegate withCloudEntity:(IMBCloudEntity *)CloudEntity {
    if (self = [super initWithDelegate:delegate]) {
       _baseManager = [[[IMBCloudManager singleton] iCloudDriveManager] retain];
    }
    return self;
}
...
- (void)changeContentViewWithDataArr:(NSMutableArray *)dataArr {
    ...
    [_dataSourAryM removeAllObjects];
...
#pragma mark - IMBDriveDelegate
//获取数据失败
- (void)loadDriveDataFail:(ActionTypeEnum)typeEnum {
        [_toolbarView toolBarButtonIsEnabled:YES];
    if (typeEnum == createFolder) {
        _newDriveModel.isForbiddden = NO;
        [_dataSourAryM removeObject:_newDriveModel];
        //提示失败
        NSString *str = CustomLocalizedString(@"CreateFolder_Fail_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:failedType];
    }else if (typeEnum == moveAction) {
        //TODO:移动失败
        NSString *str = CustomLocalizedString(@"MoveFile_Fail_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:failedType];
    }else if (typeEnum == copyAction) {
        NSString *str = CustomLocalizedString(@"copyFile_Fail_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:failedType];
    }else if (typeEnum == loadAction) {
        //TODO: HUYUMIN 显示刷新失败的页面
        [_loadingAnimationView endAnimation];
    }else if (typeEnum == deleteAction) {
        dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5/*延迟执行时间*/ * NSEC_PER_SEC));
        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
            NSString *str = CustomLocalizedString(@"deleteFile_Fail_tip_Str", nil);
            [_promptView setFillColor:[StringHelper getColorFromString:CustomColor(@"prompt_fail_bgView_fillColor", )] withBorderColor:[StringHelper getColorFromString:CustomColor(@"prompt_fail_bgView_borderColor", )]];
            [_promptView setImage:[NSImage imageNamed:@"alert_note"] WithTextString:str withIsLoading:NO withIsState:failedType];
            [self removePromptView:_promptView];
        });
    }else if (typeEnum == renameAction) {
        _renameDriveModel.isForbiddden = NO;
        NSString *str = CustomLocalizedString(@"ReName_Fail_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:failedType];
    }else if (typeEnum == loadAction) {
        //弹提示窗口  让用用户刷新界面
        [self showAlertText:CustomLocalizedString(@"Login_Registered_Exception", nil) OKButton:CustomLocalizedString(@"Button_Ok", nil)];
        [_contentBox setContentView:_noDataView];
    }
    [_collectionView reloadData];
    [_itemTableView reloadData];
...
- (void)loadDriveComplete:(NSMutableArray *_Nonnull)ary WithEvent:(ActionTypeEnum)typeEnum {
    ...
    if (typeEnum == loadAction) {
        [self changeContentViewWithDataArr:ary];
        ...
        for (NSDictionary *dic in ary) {
            [_dataSourAryM enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSString *foldID = [NSString stringWithFormat:@"%@",[dic objectForKey:@"itemIDOrPath"]];
                NSString *str = [NSString stringWithFormat:@"%@",[(IMBDriveModel *)obj fileID]];
                if ([foldID isEqualToString:str]) {
                    [_dataSourAryM removeObject:obj];
                }
            }];
        }
        [_tempDic setObject:_dataSourAryM forKey:[NSNumber numberWithInt:_doubleclickCount]];
        //提示成功
        dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5/*延迟执行时间*/ * NSEC_PER_SEC));
        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
            NSString *str = CustomLocalizedString(@"deleteFile_Complete_tip_Str", nil);
            [_promptView setImage:[NSImage imageNamed:@"alert_complete"] WithTextString:str withIsLoading:NO withIsState:successedType];
            [_promptView setFillColor:[StringHelper getColorFromString:CustomColor(@"prompt_success_bgView_fillColor", )] withBorderColor:[StringHelper getColorFromString:CustomColor(@"prompt_success_bgView_borderColor", )]];
            [_promptView setNeedsLayout:YES];
    ...
    }else if (typeEnum == createFolder) {
        _newDriveModel.isForbiddden = NO;
        IMBDriveModel *entity = [ary objectAtIndex:0];
        NSMutableArray *ary = [_tempDic objectForKey:[NSNumber numberWithInt:_doubleclickCount]];
        [ary addObject:entity];
        [_tempDic setObject:ary forKey:[NSNumber numberWithInt:_doubleclickCount]];
        //提示创建成功
        NSString *str = CustomLocalizedString(@"CreateFolder_Complete_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:successedType];
        //保存文件操作记录
        [[IMBCloudManager singleton].userTable saveOperationRecords:entity];
    }else if (typeEnum == moveAction) {
        NSMutableArray *saveAry = [[NSMutableArray alloc]init];
        [saveAry addObject:[NSNumber numberWithInt:_doubleclickCount]];
        int count = [[saveAry objectAtIndex:0] intValue];
        NSMutableArray *endSaveAry = [_tempDic objectForKey:[NSNumber numberWithInt:count]];

        if (saveAry.count >0) {
            [_dataSourAryM enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                IMBDriveModel *driveModel = obj;
                if ([ary containsObject:driveModel.itemIDOrPath]) {
                    [_dataSourAryM removeObject:driveModel];
                    [endSaveAry removeObject:driveModel];
                }
            }];
            if (endSaveAry.count > 0) {
                [_tempDic setObject:endSaveAry forKey:[NSNumber numberWithInt:_doubleclickCount]];
            }
        }
        [saveAry release];
        saveAry = nil;
        NSString *str = CustomLocalizedString(@"MoveFile_Complete_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:successedType];
        [_itemTableView reloadData];
        [_collectionView reloadData];
        //判读数据是否为空 切换界面
        ...
        //提示创建成功
        _renameDriveModel.isForbiddden = NO;
        NSString *str = CustomLocalizedString(@"ReName_Complete_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:successedType];
        [_itemTableView reloadData];
        [_collectionView reloadData];
        
        //保存文件操作记录
        [[IMBCloudManager singleton].userTable saveOperationRecords:_renameDriveModel];
    }else if (typeEnum == copyAction) {
//        NSMutableArray *saveAry = [[NSMutableArray alloc]init];
//        NSString *fileID = [NSString stringWithFormat:@"%@",_moveAndCopyFileID];
//        [_oldDocwsidDic enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
//            NSString *str = (NSString *)obj;
//            if ([str isEqualToString: fileID]) {
//                [saveAry addObject:[NSString stringWithFormat:@"%d",[key intValue]]];
//            }
//        }];
//         [saveAry addObject:[NSString stringWithFormat:@"%d",1]];
//        if (saveAry.count >0) {
//            int count = [[saveAry objectAtIndex:0] intValue];
//            NSMutableArray *endSaveAry = [_tempDic objectForKey:[NSNumber numberWithInt:count]];
//            [endSaveAry addObjectsFromArray:ary];
//            if ([_currentDriveID isEqualToString:_moveAndCopyFileID]) {
//                [_dataSourAryM addObjectsFromArray:ary];
//            }
//            [_tempDic setObject:endSaveAry forKey:[NSNumber numberWithInt:count]];
//        }
//        [saveAry release];
//        saveAry = nil;
        //提示创建成功
        NSString *str = CustomLocalizedString(@"copyFile_Complete_tip_Str", nil);
        [self loadPromtViewTitle:str withPromptEnum:successedType];
        [_itemTableView reloadData];
        [_collectionView reloadData];
    }
    [_selectAllBtn setHidden:NO];
}
