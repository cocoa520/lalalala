...
//    class_replaceMethod([_NSThemeWidgetCell class], @selector(coreUIState), (IMP)myCoreUIWidgetState, encoding);
    ...
    [_maxButton setHidden:YES];//窗口放大才显示
    ...
    [self setTrackingArea:2];
}

- (void)setTrackingArea:(int)count
{
    NSTrackingArea *trackingArea=[[NSTrackingArea alloc]initWithRect:NSRectFromCGRect(CGRectMake(0, 0, _closeButton.frame.size.width*count+10, _closeButton.frame.size.height*1)) options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:nil];
