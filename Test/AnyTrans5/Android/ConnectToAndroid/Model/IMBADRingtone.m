...
@implementation IMBADRingtone

#pragma mark - Abstract Method
- (int)queryDetailContent {
    [_loghandle writeInfoLog:@"Ringtone queryDetailContent Begin"];
    int result = 0;
    ...
    if (ret) {
        NSString *jsonStr = [self createParamsjJsonCommand:RINGTONE Operate:QUERY ParamDic:[NSDictionary dictionary]];
        ...
        if (![IMBFileHelper stringIsNilOrEmpty:jsonStr]) {
            ret = [coreSocket launchRequestContent:jsonStr FinishBlock:^(NSData *data) {
                ...
                if(msg) {
                        ...
                        for (NSDictionary *msgDic in msgArr) {
                            if ([msgDic isKindOfClass:[NSDictionary class]]) {
                                ...
                                if ([msgDic.allKeys containsObject:@"duration"]) {
                                    track.time = [[msgDic objectForKey:@"duration"] longLongValue];
                                }
                                if ([msgDic.allKeys containsObject:@"title"]) {
                                    track.title = [msgDic objectForKey:@"title"];
                                }
                                if ([msgDic.allKeys containsObject:@"data"]) {
                                    track.url = [msgDic objectForKey:@"data"];
                                }
                                if ([msgDic.allKeys containsObject:@"mime_type"]) {
                                    track.mimeType = [msgDic objectForKey:@"mime_type"];
                                }
                                if ([msgDic.allKeys containsObject:@"isDelete"]) {
                                    track.isDeleted = [[msgDic objectForKey:@"isDelete"] boolValue];
                                }
                                _reslutEntity.reslutCount ++;
                                _reslutEntity.reslutSize += track.size;
                                _reslutEntity.scanType = ScanRingtoneFile;
                    ...
                    NSLog(@"错误");
                    [_loghandle writeInfoLog:@"Ringtone queryDetailContent Error"];
                }
                [msg release];
            }];
            NSLog(@"ret:%d",ret);
    ...
    [_loghandle writeInfoLog:@"Ringtone queryDetailContent End"];
    return result;
}

- (int)exportContent:(NSString *)targetPath ContentList:(NSArray *)exportArr {
    [_loghandle writeInfoLog:@"Ringtone exportContent Begin"];
    int result = 0;
    if ([IMBFileHelper stringIsNilOrEmpty:_serialNumber]) {
        [_loghandle writeInfoLog:@"Ringtone exportContent Error -1"];
        result = -1;
    }
    if ([IMBFileHelper stringIsNilOrEmpty:targetPath]) {
        [_loghandle writeInfoLog:@"Ringtone exportContent Error -2"];
        result = -2;
    }
    ...
    if (result == 0) {
        if (ret) {
            ...
            for (IMBADAudioTrack *track in exportArr) {
                ...
                NSString *jsonStr = [self createParamsjJsonCommand:RINGTONE Operate:EXPORT ParamDic:paramDic];
                if (![IMBFileHelper stringIsNilOrEmpty:jsonStr]) {
                    NSString *exFilePath = [targetPath stringByAppendingPathComponent:[[track.title stringByReplacingOccurrencesOfString:@"/" withString:@" "] stringByAppendingPathExtension:[track.url pathExtension]]];
...
//                    _currSize += track.size;
            ...
            [_loghandle writeInfoLog:@"Ringtone exportContent Error -3"];
            result = -3;
            //关闭连接并释放端口
            [IMBSocketFactory closeDisposeSocket:scoket serialNumber:_serialNumber];
            [scoket release];
    ...
    [_loghandle writeInfoLog:@"Ringtone exportContent End"];
    return result;
}
    ...
    if (ret) {
        ...
        for (NSString *path in importArr) {
            ...
            if ([fm fileExistsAtPath:path]) {
                ...
                NSString *jsonStr = [self createParamsjJsonCommand:RINGTONE Operate:IMPORT ParamDic:paramDic];
                if (![IMBFileHelper stringIsNilOrEmpty:jsonStr]) {
                    ...
                    if (isSuccess){
                        ...
                        NSString *jsonSyncStr = [self createParamsjJsonCommand:RINGTONE Operate:SYNC ParamDic:paramSycDic];
...
- (int)deleteContent:(NSArray *)deleteArr {
    ...
    if (ret) {
        ...
        for (IMBADAudioTrack *track in deleteArr) {
            ...
            NSString *jsonStr = [self createParamsjJsonCommand:RINGTONE Operate:DELETE ParamDic:paramDic];
            if (![IMBFileHelper stringIsNilOrEmpty:jsonStr]) {
