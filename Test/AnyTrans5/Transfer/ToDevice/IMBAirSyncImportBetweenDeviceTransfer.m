        ...
        if ([keyString isEqualToString:@"media"]) {
            ...
            for (id item in items) {
                if ([item isKindOfClass:[IMBPhotoEntity class]]) {
                    ...
                    if (_isStop) {
                        [[IMBTransferError singleton] addAnErrorWithErrorName:photo.photoName WithErrorReson:CustomLocalizedString(@"ResultWindow_result_2", nil)];
                        continue;
                    }
            ...
            for (id item in items) {
                if ([item isKindOfClass:[IMBBookEntity class]]){
                    ...
                    if (_isStop) {
                        [[IMBTransferError singleton] addAnErrorWithErrorName:newItem.bookName WithErrorReson:CustomLocalizedString(@"ResultWindow_result_2", nil)];
                        continue;
                    }
...
- (NSMutableArray *)prepareMediaWithTrackArray:(NSArray *)array categoryEnum:(CategoryNodesEnum)categoryEnum withPlaylistArray:playlistArr{
    ...
    [_loghandle writeInfoLog:[NSString stringWithFormat:@"totalcount:%d",totalCount]];
    for (int i = 0; i < totalCount; i++) {
        ...
        if (!_isStop) {
            if (![TempHelper stringIsNilOrEmpty:track.title]) {
                NSString *msgStr = [NSString stringWithFormat:CustomLocalizedString(@"ImportSync_id_3", nil),track.title];
                if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
                    [_transferDelegate transferFile:msgStr];
                }
            }
            if (![self checkSupportMediaType:track.mediaType IsVideo:track.isVideo]) {
                [[IMBTransferError singleton] addAnErrorWithErrorName:track.title WithErrorReson:CustomLocalizedString(@"Ex_Op_NotSupportConvert_File", nil)];
                _skipCount += 1;
    ...
    if (array != nil && [array count] > 0) {
        ...
        if (array != nil && [array count] > 0) {
            for (IMBNewTrack *newTrack in array) {
                ...
                if (!_isStop){
                    if (![TempHelper stringIsNilOrEmpty:newTrack.title]) {
                        NSString *msgStr = [NSString stringWithFormat:CustomLocalizedString(@"ImportSync_id_3", nil),newTrack.title];
                        if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
                            [_transferDelegate transferFile:msgStr];
                        }
                    }
        ...
        @autoreleasepool {
            ...
            if (_isStop == NO) {
                ...
                if (![TempHelper stringIsNilOrEmpty:pe.photoName]) {
                    NSString *msgStr = [NSString stringWithFormat:CustomLocalizedString(@"ImportSync_id_3", nil),pe.photoName];
                    if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
                        [_transferDelegate transferFile:msgStr];
                    }
                }
                ...
                if (status == OutOfSpace) {
                    ...
                    if (![TempHelper stringIsNilOrEmpty:pe.photoName]) {
                        NSString *msgStr = [NSString stringWithFormat:CustomLocalizedString(@"MSG_TranResult_Skiped_Existed", nil),pe.photoName];
                        if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
                            [_transferDelegate transferFile:msgStr];
                        }
                    }
...
- (void)copyWordProgress:(NSString *)workStr {
    if (![TempHelper stringIsNilOrEmpty:workStr]) {
        NSString *msgStr = [NSString stringWithFormat:CustomLocalizedString(@"MSG_COM_Copying", nil),workStr];
        if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
            [_transferDelegate transferFile:msgStr];
        }
    }
