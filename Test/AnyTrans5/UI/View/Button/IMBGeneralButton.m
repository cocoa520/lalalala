...
- (void)setIconImageName:(NSString *)iconName {
    ...
    _defaultImage = [[StringHelper imageNamed:[NSString stringWithFormat:@"%@1",iconName]] retain];
    _disableImage = [[StringHelper imageNamed:[NSString stringWithFormat:@"%@2",iconName]] retain];
}
...
-(NSMutableAttributedString *)attributedTitle:(BOOL)isEntered{
    if (_buttonName) {
        ...
        if (_size) {
            [attributedTitles addAttribute:NSFontAttributeName value:[NSFont fontWithName:@"Helvetica Neue Medium" size:_size] range:NSMakeRange(0, _buttonName.length)];
        }else {
           [attributedTitles addAttribute:NSFontAttributeName value:[NSFont fontWithName:@"Helvetica Neue Medium" size:14] range:NSMakeRange(0, _buttonName.length)];
        }
    
        if (isEntered) {
            if (_buttonType == MouseEnter) {
                [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"generalBtn_enterColor", nil)] range:NSMakeRange(0, _buttonName.length)];
            }else if (_buttonType == MouseOut || _buttonType == MouseUp) {
                if (_isReslutVeiw) {
                    [attributedTitles addAttribute:NSBackgroundColorAttributeName value:[NSColor clearColor] range:NSMakeRange(0, _buttonName.length)];
                    [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"generalBtn_other_exitColor", nil)] range:NSMakeRange(0, _buttonName.length)];
                }else{
                    [attributedTitles addAttribute:NSBackgroundColorAttributeName value:[NSColor clearColor] range:NSMakeRange(0, _buttonName.length)];
                    [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"generalBtn_exitColor", nil)] range:NSMakeRange(0, _buttonName.length)];
                }
             
            }else if (_buttonType == MouseDown) {

                [attributedTitles addAttribute:NSBackgroundColorAttributeName value:[NSColor clearColor] range:NSMakeRange(0, _buttonName.length)];
                [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"generalBtn_downColor", nil)] range:NSMakeRange(0, _buttonName.length)];
            }else {
                [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"generalBtn_exitColor", nil)] range:NSMakeRange(0, _buttonName.length)];
            }
        }else {
            if (_disableColor != nil) {
                ...
                [attributedTitles addAttribute:NSForegroundColorAttributeName value:[StringHelper getColorFromString:CustomColor(@"text_disableColor", nil)] range:NSMakeRange(0, _buttonName.length)];
            }
