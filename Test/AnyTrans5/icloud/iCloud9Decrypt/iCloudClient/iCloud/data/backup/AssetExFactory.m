 ...
 *
 *  @param domain 增加一个NSString类型参数
 *
 */
+ (AssetEx*)from:(Record*)record withDomain:(NSString *)domain withProtectionZone:(ProtectionZone*)protectionZone {
    ...
    if (encryptData != nil) {
        ...
        if (decryptData != nil) {
            encryptedAttributes = [AssetExEncryptedAttributesFactory from:decryptData withDomain:domain];
        }
    }
