...
- (ECPoint*)detach {
    return [[[SecP160R2Point alloc] initWithCurve:nil withX:[self affineXCoord] withY:[self affineYCoord]] autorelease];
}
...
- (ECPoint*)add:(ECPoint*)b {
    ...
    @autoreleasepool {
        ...
        if ([Nat160 isZero:H]) {
            ...
            [SecP160R2Field multiply:[Y3 x] withY:R withZ:tt1];
            ...
            retVal = [[[SecP160R2Point alloc] initWithCurve:curve withX:X3 withY:Y3 withZS:zs withCompression:self.isCompressed] autorelease];
#if !__has_feature(objc_arc)
...
- (ECPoint*)twice {
    ...
    @autoreleasepool {
        ...
        if (!Z1IsOne) {
            Z1Squared = t2;
            [SecP160R2Field square:[Z1 x] withZ:Z1Squared];
        }
        
        [SecP160R2Field subtract:[X1 x] withY:Z1Squared withZ:t1];
        
        NSMutableArray *M = t2;
        [SecP160R2Field add:[X1 x] withY:Z1Squared withZ:M];
        [SecP160R2Field multiply:M withY:t1 withZ:M];
        ...
        NSMutableArray *zs = [[NSMutableArray alloc] initWithObjects:Z3, nil];
        retVal = [[SecP160R2Point alloc] initWithCurve:curve withX:X3 withY:Y3 withZS:zs withCompression:self.isCompressed];
#if !__has_feature(objc_arc)
        if (X3 != nil) [X3 release]; X3 = nil;
        if (Y3 != nil) [Y3 release]; Y3 = nil;
        if (Z3 != nil) [Z3 release]; Z3 = nil;
...
- (ECPoint*)negate {
    ...
    ECPoint *retVal = nil;
    @autoreleasepool {
        retVal = [[SecP160R2Point alloc] initWithCurve:[self curve] withX:[self rawXCoord] withY:[[self rawYCoord] negate] withZS:[self rawZCoords] withCompression:[self isCompressed]];
    }
    return (retVal ? [retVal autorelease] : nil);
}
