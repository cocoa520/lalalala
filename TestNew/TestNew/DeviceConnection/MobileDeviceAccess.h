...
@interface AMDiagnosticsRelayServices : AMService {
}
...
//启动开始检索的session
//参数 domainStr 可以是  @"com.apple.Contacts"，com.apple.Calendars，com.apple.Bookmarks，com.apple.Notes等
//返回值 从设备返回的对应的数组
- (id)startQuerySessionWithDomain:(NSString*)domainStr;

//启动编辑如创建，修改的session
//参数 domainStr 可以是  @"com.apple.Contacts"，com.apple.Calendars，com.apple.Bookmarks，com.apple.Notes等
//参数 anchorStr Bookmarks-Device-Anchor，Contacts-Device-Anchor，Notes-Device-Anchor
//返回值 从设备返回的对应的数组
...
typedef enum {
    FLAG_RESTORE_SYSTEM_FILES       = (1 << 1),
    FLAG_RESTORE_REBOOT             = (1 << 2),
    FLAG_RESTORE_COPY_BACKUP        = (1 << 3),
    FLAG_RESTORE_SETTINGS           = (1 << 4),
    FLAG_RESTORE_REMOVE_ITEMS       = (1 << 5),
    FLAG_ENCRYPTION_ENABLE          = (1 << 6),
    FLAG_ENCRYPTION_DISABLE         = (1 << 7),
    FLAG_ENCRYPTION_CHANGEPW        = (1 << 8),
    FLAG_FORCE_FULL_BACKUP          = (1 << 9),
    FLAG_CLOUD_ENABLE               = (1 << 10),
    FLAG_CLOUD_DISABLE              = (1 << 11)
} mobilebackup2_flags_t;
...
@class AMDevice;
@class AFCMediaDirectory;
@interface AMMobileBackupRestore : AMService {
@private
    AMDevice *_device;
    BOOL _backupServiceStatus;
    BOOL _handShakeStatus;
    NSString *_targetUUID;
    BOOL _quitFlag;
    BOOL _isMobileBackup;
    int _restoreFlags;
    BOOL _restoreReboot;
    BOOL _removeNotRestoredItems;
    NSString *_backupPassword;
//    IMBLogManager *_logHandle;
    NSCondition *_condition;
    BOOL _isOnlyManifest;
}

@property (nonatomic, readwrite) BOOL isOnlyManifest;
@property (nonatomic, readonly) BOOL backupServiceStatus;
@property (nonatomic, readonly) BOOL handShakeStatus;
@property (nonatomic, readwrite) BOOL quitFlag;
@property (nonatomic, readonly) int restoreFlags;
@property (nonatomic, getter=restoreReboot, setter=setRestoreReboot:, readwrite) BOOL restoreReboot;
@property (nonatomic, getter=removeNotRestoredItems, setter=setRemoveNotRestoredItems:, readwrite) BOOL removeNotRestoredItems;
@property (nonatomic, readwrite, retain) NSString *backupPassword;
...
- (BOOL)requestWithExcute:(ExcuteEnum)excute withBackupPath:(NSString*)backupFolder withSourceUUID:(NSString*)sourceUUID withIsFullBackup:(BOOL)isFullBackup withAFCDirectory:(AFCMediaDirectory*)AFCDirectory;
...
/**
 * RecursivveCopy the contents from a device to another device. by zhangyang 2013/05/22
 * @param frompath Full pathname of the source device file
 * @param source device app dir
 * @param topath Full pathname of the target device to copy into
 */
- (bool)recursiveCopyFile:(NSString*)srcPath sourcAFCDir:(AFCDirectoryAccess*)sourceAFCDir toPath:(NSString*)tarPath;

/**
 * Copy the contents form other device file to a file on this device.
 * @param frompath Full pathname of the device file
 * @param source AFCDir
 * @param topath Full pathname of the local file to copy into
 */
...
@property(nonatomic, readwrite) bool isWifiConnection;
@property (nonatomic,assign)BOOL isUnlock;
//是否需要输入密码
@property (readonly) bool isNeedInputPassCode;
...
/// 判断是否是通过iTunes消息通知过来的连接
@property (readonly) BOOL fromiTunes;
/// Specific class of device.  eg, "iPod1,1"
///
...
/// The same value may be retrieved by passing \p "productVersion" to \p -deviceValueForKey:
@property (readonly) NSString *productVersion;	// "Version 5.1"

@property (readonly) NSString *totalDataAvailable;
...
/// Create a mobile backup or restore.  Allows synchronisation of information
/// with the device. For more information, see AMMobileBackupRestore.
...
/// This method will be called whenever a device is need password
...
//add by zhangyang 2012/12/12 start
@property (assign) AMDevice *ConnectedDevice;
//add by zhangyang 2012/12/12 end
...
typedef NSUInteger DeviceFileType;

@interface AMFileEntity : NSObject {
    DeviceFileType _fileType;
    long _fileSize;
    NSString *_filePath;
    NSString *_name;
}

@property (assign) DeviceFileType FileType;
...
@property (assign) BOOL isSqliteEnd;
@property (assign) BOOL skipScan;
@property (assign) BOOL stopScan;
@property (assign) BOOL stopClean;
@property (assign) BOOL isPause;
+(ScanStatus *)shareInstance;
@end

@interface AsyncState : NSObject {
    int _offset;
    NSCondition *_condition;
    NSFileHandle *_fileHandle;
    AFCFileReference *_dfHandle;
    NSMutableArray *_bufferList;
    NSMutableArray *_readLengthList;
...
@property (nonatomic, readwrite) uint32_t bufsz;
@property (nonatomic, assign) char *buff;
@property (nonatomic, retain) AFCFileReference *dfHandle;
@property (nonatomic, retain) NSCondition *condition;
@property (nonatomic, readwrite) int offset;
@property (nonatomic, retain) NSFileHandle *fileHandle;
@property (nonatomic, retain) NSMutableArray *bufferList;
@property (nonatomic, retain) NSMutableArray *readLengthList;
@property (nonatomic, readwrite) long long fileLength;
@property (nonatomic, readwrite) BOOL isReaded;
@property (nonatomic, readwrite) int writeCountOnce;
