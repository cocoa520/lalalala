...
- (void)textDidChange:(NSNotification *)notification{
    NSTextField *tf = [[notification object] retain];
    if ([self.dlg respondsToSelector:@selector(inputTFDidChange:)]) {
        [self.dlg inputTFDidChange:tf];
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFY_TEXTFILED_INPUT_CHANGE object:self userInfo:@{@"text" : tf.stringValue}];
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFY_REGISTER_TEXTFILED_INPUT_CHANGE object:self userInfo:@{@"text" : tf.stringValue}];
}
