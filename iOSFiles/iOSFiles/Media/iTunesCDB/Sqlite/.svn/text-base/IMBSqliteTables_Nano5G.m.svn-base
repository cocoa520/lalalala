...
@implementation IMBSqliteTables_Nano5G
...
- (void)readLookupTables {
    ...
    query = @"select pid, ifnull(artist_pid,0), name,is_unknown,has_songs,has_music_videos from album order by pid";
    rs = [_libraryConnection executeQuery:query];
    while ([rs next]) {
        long long iD = [rs longLongIntForColumn:@"pid"];
...
- (void)createPlaylist:(FMDatabase*)db playList:(IMBPlaylist*)playlist {
    ...
    [_dynamicConnection executeUpdate:insertCmd withParameterDictionary:params];
