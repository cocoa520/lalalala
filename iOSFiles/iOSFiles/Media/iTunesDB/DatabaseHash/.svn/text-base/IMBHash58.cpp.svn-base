//
//  IMBHash58.cpp
//  iMobieTrans
//
//  Created by Pallas on 1/15/13.
//  Copyright (c) 2013 iMobie Inc. All rights reserved.
//

#include "IMBHash58.h"
#include <cstdio>
#include <cstring>
#include <openssl/sha.h>

typedef unsigned char uint8_t;
typedef unsigned uint32_t;
typedef unsigned long long uint64_t;

const uint8_t table1[] = {
	
},
table2[] = {
	
},
invTable[] = {
	
},
fixed[] = {
    0x67, 0x23, 0xFE, 0x30, 0x45, 0x33, 0xF8, 0x90, 0x99, 0x21, 0x07, 0xC1, 0xD0, 0x12, 0xB2, 0xA1, 0x07, 0x81
};

int gcd(int a, int b) {
    while (true) {
        a = a % b;
        if (a == 0) {
            return b;
        }
        b = b % a;
        if (b == 0) {
            return a;
        }
    }
}

int lcm(int a, int b) {
    if (a==0 || b==0) {
        return 1;
    }
    return ((a * b) / gcd(a, b));
}

void generateKey(uint8_t *fwid, uint8_t key[64]) {
    if (sizeof(fwid) == 8) {
        return;
    }
    
    uint8_t y[16] = { 0x00 };
    for (int i = 0; i < 4; i++)
    {
        int _lcm = lcm((int)fwid[i * 2], (int)fwid[i * 2 + 1]);
        uint8_t hi = (uint8_t)((_lcm & 0xFF00) >> 8);
        uint8_t lo = (uint8_t)(_lcm & 0xFF);
        
        y[i * 4] = (uint8_t)((table1[hi] * 0xB5) - 3);
        y[i * 4 + 1] = (uint8_t)((table2[hi] * 0xB7) + 0x49);
        y[i * 4 + 2] = (uint8_t)((table1[lo] * 0xB5) - 3);
        y[i * 4 + 3] = (uint8_t)((table2[lo] * 0xB7) + 0x49);
    }
    
    for (int i = 0; i < 16; i++)
    {
        y[i] = invTable[y[i]];
    }
    SHA_CTX content;
    SHA1_Init(&content);
    SHA1_Update(&content, fixed, 18);
    SHA1_Update(&content, (uint8_t*)y, 16);
    SHA1_Final(key, &content);
}

void hash58(uint8_t buf[], int flen, uint8_t signature[20], uint8_t *fwid) {
    uint8_t key[64] = { 0x00 };
    uint8_t hash[20] = {0x00 };
    generateKey(fwid, key);
    for (int i = 0; i < 64; i++) {
        key[i] ^= 0x36;
    }
    SHA_CTX content;
    SHA1_Init(&content);
    SHA1_Update(&content, key, 64);
    SHA1_Update(&content, buf, flen);
    SHA1_Final(hash, &content);
    
    for (int i = 0; i < 64; i++) {
        key[i] ^= 0x36 ^ 0x5c;
    }
    SHA_CTX resultContent;
    SHA1_Init(&resultContent);
    SHA1_Update(&resultContent, key, 64);
    SHA1_Update(&resultContent, hash, 20);
    SHA1_Final(signature, &resultContent);
}

void Hash58File(uint8_t *buf, int len, uint8_t signature[20], const char fwid_hex[]) {
    uint64_t id = 0, k;
    uint8_t fwid[sizeof(id)] = { 0x00 };
    for(int i = 0;i < 8;++i) {
        sscanf(fwid_hex + i*2, "%02llX", &k);
        id |= k<<(i*8);
    }
    for (int i = 0; i < sizeof(id); i++) {
        fwid[i] = id>>(i*8) & 0xFF;
    }
    hash58(buf, len, signature, fwid);
}