...
- (id)init {
    ...
    if (self) {
        ...
        _productName = @"AllFiles";
        _resProductName = @"AllFiles";
        _buildDate = @"20180410";
...
//        [[NSUserDefaults standardUserDefaults] setObject:@"blackSkin" forKey:@"customColor"];
        ...
        if (skinPlistPath == nil || ![fm fileExistsAtPath:skinPlistPath]) {
            [self setCurUseSkin:@"christmasSkin"];//默认设置为圣诞节皮肤   whiteSkin
            [[NSUserDefaults standardUserDefaults] setObject:@"christmasSkin" forKey:@"customColor"];
            ...
            if ([StringHelper stringIsNilOrEmpty:curSkin]) {
                [self setCurUseSkin:@"christmasSkin"];
            } else {
...
- (NSString*) getProductTitle {
    if (!_isRegistered ) {
        return [NSString stringWithFormat:@"%@ - Unregistered", self.productName];
    } else {
