...
- (void)startDelete
{
    ...
    for (IMBPlaylist *playlist in _deleteArray) {
        ...
        if (![TempHelper stringIsNilOrEmpty:playlist.name]) {
            if ([_delegate respondsToSelector:@selector(setDeleteProgress:withWord:)]) {
                [_delegate setDeleteProgress:((float)curIndex/_deleteArray.count)*96 withWord:[NSString stringWithFormat:@"Deleting %@...",playlist.name]];
            }
        }
        @try {
    ...
    //开始同步
    if ([_delegate respondsToSelector:@selector(setDeleteProgress:withWord:)]) {
        [_delegate setDeleteProgress:((float)curIndex/_deleteArray.count)*96 withWord:@"Preparing to sync, please wait..."];
    }
