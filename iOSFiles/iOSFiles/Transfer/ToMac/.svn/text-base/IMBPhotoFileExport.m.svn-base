...
- (void)startTransfer {
    ...
    for (DriveItem *driveItem in _exportTracks) {
        if (driveItem.isStart) {
            return;
        }
        if (driveItem.dataAry.count>0) {
            if (driveItem.dataAry != nil && driveItem.dataAry.count > 0 && ![TempHelper stringIsNilOrEmpty:_exportPath]) {
                ...
                //计算totalSize
                _totalSize = [self caculateTransferTotalSize:driveItem.dataAry];
                ...
                for (IMBPhotoEntity *pe in driveItem.dataAry) {
                    ...
                    if (!_isStop) {
                        ...
                        if ([_transferDelegate respondsToSelector:@selector(transferProgress:)]) {
                            float progress = (float)_successCount/(float)driveItem.dataAry.count*100;
