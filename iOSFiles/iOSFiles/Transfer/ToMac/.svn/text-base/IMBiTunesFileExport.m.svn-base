...
//            }
            ...
            if (!_isStop) {
                ...
                if (![TempHelper stringIsNilOrEmpty:itunesFilePath]) {
                    NSString *msgStr = [NSString stringWithFormat:@"Copying %@...",[itunesFilePath lastPathComponent]];
...
//                        [_limitation reduceRedmainderCount];
                        ...
                        [[IMBTransferError singleton] addAnErrorWithErrorName:track.name WithErrorReson:@"Coping file failed."];
                        _failedCount += 1;
                    }
                } else {
                    [[IMBTransferError singleton] addAnErrorWithErrorName:track.name WithErrorReson:@"The file does not exist in your iPhone or your backups"];
                    _failedCount += 1;
                }
            }else {
                [[IMBTransferError singleton] addAnErrorWithErrorName:track.name WithErrorReson:@"Skipped"];
                _skipCount ++;
            }
        }
    }
