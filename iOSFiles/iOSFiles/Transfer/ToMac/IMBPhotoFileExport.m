...
- (void)startTransfer {
    ...
    for (DriveItem *pe in _exportTracks) {
        if (pe.isStart) {
            continue;
        }
...
//            _totalSize = [self caculateTransferTotalSize:_exportTracks];
            ...
            if (!_isStop) {
                ...
                if (![TempHelper stringIsNilOrEmpty:pe.fileName]) {
                    NSString *msgStr = [NSString stringWithFormat:@"Copying %@...",pe.fileName];
                    if ([_transferDelegate respondsToSelector:@selector(transferFile:)]) {
                        [_transferDelegate transferFile:msgStr];
                    }
                }
                NSString *filePath = [_exportPath stringByAppendingPathComponent:pe.fileName];
                NSString *nowPath = @"";
                BOOL success = NO;
                if (_exportType == 1) {//导出设备中的图片
                    ...
                    if ([_ipod.fileSystem fileExistsAtPath:remotingFilePath]) {
                        ...
                        if (success) {
                            ...
                            pe.state = DownloadStateError;
                            _failedCount += 1;
                        }
                    }else {
                        [[IMBTransferError singleton] addAnErrorWithErrorName:pe.fileName WithErrorReson:@"The file does not exist in your iPhone or your backups"];
                ...
                [[IMBTransferError singleton] addAnErrorWithErrorName:pe.fileName WithErrorReson:@"Skipped"];
                _skipCount ++;
            }
