...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    if (_state == 2) {
        [COLOR_DOENLOAD_DETAIL_ENTERBGCOLOR setFill];
    }else if (_state == 3) {
        [COLOR_DOENLOAD_DETAIL_ENTERBGCOLOR setFill];
    }else{
...
- (void)setBadgeCount:(NSInteger)badgeCount
{
    ...
    [str addAttribute:NSForegroundColorAttributeName value:COLOR_PROMPTBTN_NORMAL range:NSMakeRange(0, str.length)];
    NSRect rect;
    if (_badgeCount != 0) {
        if (str.length == 1) {
            if (_isiCloudDownLoadView) {
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:badgecountStr?badgecountStr:@""];
    [str addAttribute:NSFontAttributeName value:[NSFont boldSystemFontOfSize:10] range:NSMakeRange(0, str.length)];
    [str addAttribute:NSForegroundColorAttributeName value:COLOR_BUTTON_SEGDOWN range:NSMakeRange(0, str.length)];
    NSRect textrect;
